...
namespace DataAcces.Repositories
{
    public class RepositorioReserva: RepositorioGeneral<Reserva>, IRepositorioReserva
    {
        ...
        {
            return Contexto.Set<Reserva>().Include(r => r.Cliente).Include(r =>r.Servicio).Where(c => c.Fecha.Date == fecha.Date).ToList();
        }

        public IEnumerable<Reserva> BuscarPorNombreCliente(string nombre)
        {
          
                return Contexto.Set<Reserva>().Include(r => r.Cliente).Include(r => r.Servicio).AsNoTracking().AsEnumerable().Where(r => r.Cliente?.Nombre  == nombre || r.NombreCliente == nombre).ToList();
            
        }

        public IEnumerable<Reserva> BuscarPorNombreServicio(string nombre)
        {
            return Contexto.Set<Reserva>().Include(r => r.Cliente).Include(r => r.Servicio).AsNoTracking().AsEnumerable().Where(c => c.ObtenerNombreServicio() == nombre).ToList();
        }
